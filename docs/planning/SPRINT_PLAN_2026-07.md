...
Crawl is **trunk-based on `main`**. Ticket branches merge directly to `main` — there is no release branch.

- Each ticket gets its own branch off `main` — `fix/vote-state-persistence`, `fix/filter-predicates`, `fix/card-map-layout`, `chore/version-sync`, `feature/splash-logo`, etc. — PR'd directly back into `main`.
- After merge, PR triggers `staging-build.yml` (TestFlight staging build) and `release-version.yml` (Changesets Version PR).
- When changesets' "chore(release): version packages" PR merges, versions are bumped and CHANGELOGs written.
- A human then pushes a git tag (`api-vX.Y.Z` or `mobile-vX.Y.Z`) to release. The tag push triggers the release workflow.
- Each ticket below records `main` as its target branch.
...
**Ticket 0 — Re-anchor to the real 1.0.0(13) baseline and stop config drift** · `chore` · Branch: `chore/version-sync` off `main` · [#44](https://github.com/tyler-teufel/crawl/issues/44)
...
**Ticket 1 — Fix daily vote count reset** · `bug` · Branch: `fix/vote-state-persistence` off `main` · [#45](https://github.com/tyler-teufel/crawl/issues/45)
...
**Ticket 2 — Fix broken filtering** · `bug` · Branch: `fix/filter-predicates` off `main` · [#46](https://github.com/tyler-teufel/crawl/issues/46)
...
**Ticket 3 — Shrink venue cards, fix map/carousel padding** · `bug`/`improvement` · Branch: `fix/card-map-layout` off `main` · [#47](https://github.com/tyler-teufel/crawl/issues/47)
...
**Ticket 4 — Logo + animated splash** · `feature` · Branch: `feature/splash-logo` off `main` · [#48](https://github.com/tyler-teufel/crawl/issues/48)
...
**Ticket 5 — Font exploration & onboarding/login UI refresh** · `feature` · Branch: `feature/onboarding-refresh` off `main` · [#49](https://github.com/tyler-teufel/crawl/issues/49)
...
**Ticket 6 — Build Global Rankings screen** · `feature` · Sprint 3, v1.2.0 · Branch: `feature/global-rankings` off `main` · [#50](https://github.com/tyler-teufel/crawl/issues/50)
...
**Ticket 7 — Build Profile screen** · `feature` · Sprint 4, v1.3.0 · Branch: `feature/profile-screen` off `main` · [#51](https://github.com/tyler-teufel/crawl/issues/51)
...
**Ticket 8 — Sentry not operational: dashboard still in setup state** · `bug` · Investigation-first, then fix · Branch: `claude/sentry-setup-investigation-vejzoz` off `main` (ships app code — `sentry-verify.ts` + a `_layout.tsx` effect — under a `@crawl/mobile` patch changeset) · [#57](https://github.com/tyler-teufel/crawl/issues/57)
