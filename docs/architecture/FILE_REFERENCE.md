...
| `metro.config.js`     | Metro bundler configuration. Calls `getSentryExpoConfig()` (drop-in replacement for `getDefaultConfig` that adds Sentry's source-map-friendly serializer) and wraps the result with `withNativeWind()` which processes `global.css` and sets `inlineRem: 16` (converts rem units to 16px base for React Native).                                                                                                                                                                                                                     |
| `tailwind.config.js`  | Tailwind CSS configuration with NativeWind preset. Defines two color systems: the `crawl-*` custom palette (hardcoded hex) and semantic tokens mapped from CSS variables (`bg-primary`, etc.). Defines `fontFamily` classes (`font-display`, `font-display-medium`, `font-display-bold`, `font-sans`, `font-sans-medium`, `font-sans-bold`) mapped to the Clash Grotesk / Satoshi custom font files loaded in `app/_layout.tsx`. Includes `borderWidth: hairline` utility, accordion keyframes/animations, and `tailwindcss-animate` plugin. Content array scans `app/`, `components/`, `src/`, and `@rnr` node_modules. Dark mode set to `class`. |
| `assets/fonts/`       | Clash Grotesk (Medium/SemiBold/Bold) and Satoshi (Regular/Medium/Bold) `.otf` files plus their Fontshare license files. Loaded via `useFonts` in `app/_layout.tsx`, exposed to the app as NativeWind `font-*` classes (see `tailwind.config.js`).                                                                                                                                                             |
...
- **Calls `initSentry()`** at module load (before component render) and exports the layout via `Sentry.wrap(RootLayout)` so unhandled JS / React render errors are captured
...
- `readSessionWithTimeout()` — races `supabase.auth.getSession()` against `SESSION_READ_TIMEOUT_MS` (5s) and resolves `false` on expiry, so a stalled token refresh can't hold the gate in `'loading'` forever (#191). Treating a timeout as "no returning session" falls through to the flag's answer — the pre-#158 degraded behavior — rather than stranding the user on a blank screen. Reports both the timeout and a thrown read to Sentry.
...
Wraps `@sentry/react-native` for the app. Exports:
...
- `0007_cast_vote_rpc.sql` — Vote-day boundary alignment and vote-cap enforcement: `public.vote_day(at, tz)` (canonical 04:00-timezone vote-day boundary, synced with TypeScript `voteDayFor`), `public.cast_vote(p_venue_id)` (SECURITY DEFINER RPC enforcing 3-vote/day cap + per-venue dedup, advisory-locked for concurrency), and re-points `recalculate_hotspot_scores()` to use the canonical vote_day (#126, #64).
- `0008_get_vote_state_rpc.sql` — Extracts vote-cap and vote-reset helpers as pure SQL functions (`vote_max_daily_votes()`, `vote_reset_at(at, tz)`), re-points `cast_vote()` to use them via CREATE OR REPLACE, and creates `get_vote_state()` (SECURITY INVOKER RPC for reading the caller's vote state via RLS) (#196).
- `0009_fix_handle_new_auth_user_search_path.sql` — Pins `pg_temp` on `handle_new_auth_user()`'s `search_path` via CREATE OR REPLACE, closing a temp-table-shadowing gap in the auth trigger (#189).
