...
**Why `DIRECT_URL`, not `DATABASE_URL`:** `apps/api/drizzle.config.ts` requires `DIRECT_URL` (the Supabase direct/session-pooler connection, port 5432) whenever `NODE_ENV !== 'development'`, and throws otherwise. This job sets `NODE_ENV=production` on each step that touches `drizzle.config.ts` (list pending, `drizzle-kit check`, apply, report ledger) specifically to keep that guard live, so a missing `DIRECT_URL` secret on the `database` Environment fails the run loudly instead of silently falling back to `DATABASE_URL` — the transaction pooler (port 6543), which breaks DDL and session-scoped features some migrations need and only fails "on later runs" in a way that's hard to trace back to this. `NODE_ENV` is deliberately *not* set at the job level: job-level `env` applies to every step, including `Install deps` (`npm ci`), and npm skips installing `devDependencies` — which is where `drizzle-kit` lives — whenever `NODE_ENV=production` is set, causing `Install deps` to silently succeed without `drizzle-kit` and the next step to fail with `drizzle-kit: not found` (see the `release-api.yml` note directly above for what that failure mode actually looks like in practice, and #205 for this workflow's own instance of it).
